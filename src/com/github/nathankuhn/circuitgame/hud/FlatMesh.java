...
public class FlatMesh {
    ...
    public static Mesh BuildTextMesh(Vector2f center, char[][] chars, Font font, float zOffset) {

        Vector2f size = new Vector2f(0.1f, 0.1f); // TODO figure out how to set this according to the wind

        int rows = chars.length;
        int cols = chars[0].length;

        float originX = center.x - (size.x * cols) / 2;
        float originY = center.y - (size.y * rows) / 2;
        ...
        for (int x = 0; x < cols; x++) {
            for (int y = 0; y < rows; y++) {
                ...
                currentCell = x + y * cols;
                verts[currentCell * 4] =     new Vector3f(x       * size.x + originX, y       * size.y + originY, zOffset);
                verts[currentCell * 4 + 1] = new Vector3f(x       * size.x + originX, (y + 1) * size.y + originY, zOffset);
                verts[currentCell * 4 + 2] = new Vector3f((x + 1) * size.x + originX, (y + 1) * size.y + originY, zOffset);
                verts[currentCell * 4 + 3] = new Vector3f((x + 1) * size.x + originX, y       * size.y + originY, zOffset);
